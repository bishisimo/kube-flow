...
#[tauri::command]
pub fn security_set_auto_lock_minutes(minutes: u32) -> Result<(), String> {
    let mut cfg = load_settings()?;
...
#[tauri::command]
pub fn stronghold_initialize(master_password: String, manager: State<'_, CredentialManager>) -> Result<(), String> {
    manager.stronghold_initialize(&master_password)
}

/// 解锁 Stronghold：输入主密码，将凭证加载到内存。
#[tauri::command]
pub fn stronghold_unlock(master_password: String, manager: State<'_, CredentialManager>) -> Result<(), String> {
    manager.stronghold_unlock(&master_password)
}

/// 锁定 Stronghold：清除内存中的解密数据，需重新输入主密码才能访问凭证。
#[tauri::command]
pub fn stronghold_lock(manager: State<'_, CredentialManager>) {
