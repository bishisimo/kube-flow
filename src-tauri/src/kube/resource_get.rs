...
pub async fn get_resource_value(
    ...
    match kind {
        ...
        "PersistentVolume" => {
            get_and_serialize(Api::<PersistentVolume>::all(client.clone()), name).await
        }
        ...
        "ClusterRoleBinding" => {
            get_and_serialize(Api::<ClusterRoleBinding>::all(client.clone()), name).await
        }
        "Pod" => {
            get_ns(
                Api::<Pod>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "Deployment" => {
            get_ns(
                Api::<Deployment>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "Service" => {
            get_ns(
                Api::<Service>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "StatefulSet" => {
            get_ns(
                Api::<StatefulSet>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "DaemonSet" => {
            get_ns(
                Api::<DaemonSet>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "ConfigMap" => {
            get_ns(
                Api::<ConfigMap>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "Secret" => {
            get_ns(
                Api::<Secret>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        ...
        "Endpoints" => {
            get_ns(
                Api::<Endpoints>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        ...
        "Role" => {
            get_ns(
                Api::<Role>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "RoleBinding" => {
            get_ns(
                Api::<RoleBinding>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "ReplicaSet" => {
            get_ns(
                Api::<ReplicaSet>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "Job" => {
            get_ns(
                Api::<Job>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "CronJob" => {
            get_ns(
                Api::<CronJob>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        "Ingress" => {
            get_ns(
                Api::<Ingress>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
        ...
        "LimitRange" => {
            get_ns(
                Api::<LimitRange>::namespaced(client.clone(), ns_or_default(namespace)),
                name,
            )
            .await
        }
